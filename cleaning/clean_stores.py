...
def clean_table(df: pd.DataFrame, schema: dict) -> pd.DataFrame:
    ...
    # -----------------------------
    # Square_Meters validation & missing value imputation
    # -----------------------------
    if "square_meters" in df.columns:
        median_value = df["square_meters"].median()
        df["square_meters"] = df["square_meters"].fillna(median_value)
        # Flag rows that were zero or missing initially
        df["square_meters_flagged"] = False
        df.loc[df["square_meters"] <= 0, "square_meters_flagged"] = True

    # -----------------------------
    # Open_Date validation
    # -----------------------------
    if "open_date" in df.columns:
        today = pd.Timestamp(datetime.today().date())
        df["open_date_flagged"] = False
        # flag missing or future dates
        df.loc[df["open_date"].isna() | (df["open_date"] > today), "open_date_flagged"] = True
        # forward/backward fill missing dates
        df["open_date"] = df["open_date"].ffill().bfill()
...
def main():
    ...
    SCHEMA_PATH = os.path.join(BASE_DIR, "schema", "stores_schema.yml")
    TABLE_NAME = "stores"
    ...
    log_changes(df_raw, df_clean,stage='STORE_CLEANING')

    logger.info(f"Stores cleaning pipeline completed successfully.")
